...
class Validator {
	...
	private static final Pattern PATTERN_DESCRIPTION = Pattern.compile("^[^\\n]{10,500}$");                                     //valida a maior parte dos caracteres, minimo de caracteres 10 maximo 255
	private static final Pattern PATTERN_UNICODE = Pattern.compile("^[\\p{L}][\\p{L}\\s]*[\\p{L}]{10,500}$");					//valida todos os caracteres UNICODE, minimo de caracteres 10 maximo 255
    private static final Pattern PATTERN_CATEGORY = Pattern.compile("^(HARD|SOFT)$");											//XXX solução temporaria 
    private static final Pattern PATTERN_STATUS = Pattern.compile("^(active|on_leave|inactive)$");														//XXX solução temporaria
	...
	public static boolean categoryValidateString(String category) {
		return PATTERN_CATEGORY.matcher(category).matches();
	}
	
	public static boolean statusValidateString(String Status) {
		return PATTERN_STATUS.matcher(Status).matches();
	}
